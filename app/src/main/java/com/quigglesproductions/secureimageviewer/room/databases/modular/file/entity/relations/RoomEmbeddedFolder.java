...
public class RoomEmbeddedFolder implements IDisplayFolder, IDatabaseFolder {
    @Embedded
    public RoomModularFolder folder;
    @Relation(parentColumn = "FolderId",entityColumn = "FolderId",entity = RoomModularFile.class)
    public List<RoomEmbeddedFile> files;
    @Relation(parentColumn = "OnlineThumbnailId",entityColumn = "OnlineId",entity = RoomModularFile.class)
    ...
    public IFolderDataSource getDataSource() {
        if(folder != null && folder.getDataSource() == null)
            setDataSource(new RoomModularFolderDataSource(this));
